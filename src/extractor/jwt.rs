    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self, Self::Rejection> {
        let auth_header = parts.headers
            .get(AUTHORIZATION)
            .ok_or_else(|| get_error(parts, api_error!(Unauthorized)))?
            .to_str()
            .map_err(|_| get_error(parts, api_error!(Unauthorized)))?;

        if !auth_header.starts_with("Bearer ") {
            return Err(get_error(parts, api_error!(Unauthorized)));
        }
        ...
        let data: JwtPayload = serde_json::from_slice(&decoded_bytes).map_err(|e| {
            get_error(parts, api_error!(Unauthorized, &format!("Invalid JWT payload: {}", e)))
        })?;

        let user_id = data.sub.parse::<uuid::Uuid>().map_err(|_| {
            get_error(parts, api_error!(BadRequest, "Invalid user ID format in JWT"))
        })?;
